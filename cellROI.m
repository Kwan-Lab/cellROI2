...
% CONTRIBUTORS: Michael Siniscalchi, Hongli Wang, Neil Savalia, Alex Kwan
%
%***FUTURE EDITS:
...
% --- Executes on button press in radiobutton_loadFrame.
function radiobutton_loadFrame_Callback(hObject, eventdata, handles)
% load one single frame
handles.img{1} = getProjection(1,handles); %img = getProjection(axisID,handles)
guidata(hObject, handles); %Save user data
refresh_Axis(1,handles);

function edit_loadFrame_Callback(hObject, eventdata, handles)

% --- Executes during object creation, after setting all properties.
function edit_loadFrame_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

% Hint: get(hObject,'Value') returns toggle state of radiobutton_loadFrame
...
%Refresh axes
%handles.curr_ROI = [];
% refresh_Axis((1:4),handles); % don't refresh axes before ROI is drawn -- this keeps any user-defined zooming in place

%Get arbitrary polygonal ROI from user input
BW = drawpolygon(handles.axes1, 'LineWidth', .5);
handles.cellMask = BW.createMask; %Get logical mask approximation of polygon drawn by user
...
% --- Executes on button press in grid_ROI.
function pushbutton_gridROI_Callback(hObject, eventdata, handles)
...
%Save data for current ROI
    ...
    for tt = 1:size(handles.cellMask,3)
        ...
        handles.check_overwrite = true; 
        
    %Save data for current ROI
        S = struct('cellf',handles.curr_cellf,'bw',handles.cellMask(:,:,tt));
        save(fullfile(handles.save_dir,save_name),'-struct','S');
    end
end
...
else % for grid ROI selection
    handles.cellMasksAll = handles.cellMask;
    for tt = 1:size(handles.curr_ROI,3)
        handles.roi{tt} = handles.curr_ROI(:,:,tt);
        handles.cellf{tt} = handles.curr_cellf;  % save the empty curr_cellf for now
    end
end
guidata(hObject, handles);
...
uicontrol(handles.grid_ROI); %Focus on button (clear edit cursor)
...
function edit_gridWidth_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
...
if axisID == 1
    radioOn = find([handles.radiobutton_meanAxis1.Value,...
        ...
        handles.radiobutton_loadFrame.Value]);
...
switch radioOn
    ...
    case 5
        currFrame = str2double(handles.edit_loadFrame.String);
        img = handles.stack(:,:,currFrame);
end
%Adjust White Level
        ...
        if handles.checkbox_showROIs.Value
            ...
            if ~currIdx && ~isempty(handles.curr_ROI) %If newly drawn ROI
                if size(handles.curr_ROI,3) == 1 % for ROI selection methods that select one ROI at a time
                    ...
                    for tt = 1:wid
                        patch(handles.(ax{i}),'XData',handles.curr_ROI(:,1,tt+wid*(tt-1)),'YData',handles.curr_ROI(:,2,tt+wid*(tt-1)),...
                            'FaceColor','none','EdgeColor','y','LineWidth',3,'EdgeAlpha',0.8); 
                        hold on;
                    end
                end
...
if size(handles.cellMask,3) == 1  % for ROI selection methods that select one ROI at a time
...
else  % for ROI selection methods that select multiple ROIs (grid method)
    bounds_temp = bwboundaries(handles.cellMask(:,:,1)); %Boundaries of logical mask for current ROI
    roi_temp = [bounds_temp{1}(:,2) bounds_temp{1}(:,1)]; %Boundary coordinates in ij
    roi = zeros(size(roi_temp,1), size(roi_temp,2), size(handles.cellMask,3));
    %Get cellF as mean per-pixel fluorescence and plot the dF/F on bottom panel
    cellf = NaN(size(handles.stack,3),size(handles.cellMask,3));  % empty variable 
    for jj=1:size(handles.cellMask,3)
        bounds = bwboundaries(handles.cellMask(:,:,jj));
